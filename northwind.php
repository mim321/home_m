...
if(isset($_GET["t"])) {
  ...
  where `orderdetails`.unitprice > 100
  "; break;    
  ...
  where `orderdetails`.unitprice > 100 and orders.orderid = `orderdetails`.orderid
  "; break;   
  /* 14 - inner join 2 way */
  case "14": $sql="
  select orders.customerid, `orderdetails`.*, products.*
  ...
  case "15": $sql="
  select orderid, sum(quantity * unitprice) as total from `orderdetails`
  group by orderid
